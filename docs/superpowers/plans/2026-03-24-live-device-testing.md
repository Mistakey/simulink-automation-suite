# Live Device Testing Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a project-level skill that drives AI to perform end-to-end testing of the simulink-automation-suite plugin on real MATLAB/Simulink hardware, producing traceable incremental test reports.

**Architecture:** A single SKILL.md playbook defines the testing methodology (phases, report format, quality standards) while the AI dynamically generates test cases from the CLI's `schema` action output. The skill enforces behavioral isolation — AI must not read source code, only interact through the plugin's published interface.

**Tech Stack:** Markdown (skill definition), Bash (CLI execution), Python (simulink_cli invocation)

**Spec:** `docs/superpowers/specs/2026-03-24-live-device-testing-design.md`
...
|------|--------|----------------|
| `.claude/skills/live-testing/SKILL.md` | Create | Full testing playbook — isolation rules, phases, report format, incremental logic |
| `.claude/CLAUDE.md` | Modify | Add `/live-test` command reference after existing commands |
| `.claude/skills/release/SKILL.md` | Modify | Insert report archive step between "Validate locally" and "Commit release changes" |

---

### Task 1: Create the live-testing SKILL.md

**Files:**
- Create: `.claude/skills/live-testing/SKILL.md`

- [ ] **Step 1: Create the skill file**

Create `.claude/skills/live-testing/SKILL.md` with the following complete content:
...
Run:
```bash
head -5 .claude/skills/live-testing/SKILL.md
```
Expected: YAML frontmatter with `name: live-test` and `description:` fields.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/live-testing/SKILL.md
git commit -m "feat: add live device testing skill for real MATLAB/Simulink verification"
```

---

### Task 2: Update CLAUDE.md with /live-test command reference

**Files:**
- Modify: `.claude/CLAUDE.md`

- [ ] **Step 1: Add /live-test to Commands section**

Inside the bash code block, after the line `python -m simulink_cli --json '{"action":"schema"}'` and before the closing triple-backtick, add:
...
- [ ] **Step 2: Add /live-test to On-Demand Context section**

After the line beginning with `- **\`/release\` skill**`, add:
...
git commit -m "docs: add /live-test command reference to CLAUDE.md"
```

---

### Task 3: Update release skill with report archive step

**Files:**
- Modify: `.claude/skills/release/SKILL.md`

- [ ] **Step 1: Insert archive step between step 3 and step 4 in Release Flow**

After the closing triple-backtick of the "Validate locally" code block, before the line `4. Commit release changes`, insert:
...
Renumber subsequent steps: current step 4 → 5, step 5 → 6, step 6 → 7. Scan the rest of the document for any references to step numbers 4/5/6 and update accordingly.

- [ ] **Step 2: Add report check to Agent Checklist**

After the existing checklist item `- [ ] No stale docs describing manual-only release flow`, add:
...
git commit -m "docs: add live test report archive step to release workflow"
```
...
- [ ] **Step 1: Verify all three files are committed**

```bash
git log --oneline -5
```

Expected: 3 new commits for the skill, CLAUDE.md update, and release skill update.

- [ ] **Step 2: Verify skill is discoverable**

```bash
ls .claude/skills/live-testing/SKILL.md
```
