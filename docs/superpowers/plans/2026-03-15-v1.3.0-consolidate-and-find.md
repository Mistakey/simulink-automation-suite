...
| `skills/_shared/validation.py` | `validate_text_field()`, `_invalid_input()` |
...
Extract `_invalid_input` and `validate_text_field` from `sl_core.py`:
...
Run: `python -c "from skills._shared.validation import validate_text_field; print('OK')"`
...
from skills._shared.errors import make_error
from skills._shared.validation import validate_text_field, _invalid_input
from .sl_scan import (
...
- [ ] **Step 2: Remove `_invalid_input` and `validate_text_field` function definitions from `sl_core.py`**

Delete lines 259-282 (the `_invalid_input` and `validate_text_field` function definitions), since they now come from `_shared/validation.py`.
...
class FakeFindEngine:
    ...
    def get_param(self, path, param):
        if param == "Handle":
            ...
            return 1.0
        if param == "BlockType":
            # Return a predictable type based on path
...
| `skills/_shared/errors.py` | `make_error()` — error envelope builder. |
| `skills/_shared/json_io.py` | `JsonArgumentParser`, `emit_json()`, `as_list()`, `project_top_level_fields()`. |
| `skills/_shared/validation.py` | `validate_text_field()`, `_invalid_input()` — input hardening. |
