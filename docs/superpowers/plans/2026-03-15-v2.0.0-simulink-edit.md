...
**Known tech debt:** `_parse_json_request`, `_json_request_to_argv`, `parse_request_args`, `map_value_error`, and `map_runtime_error` are duplicated between `simulink_scan` and `simulink_edit` `sl_core.py` files. Per spec, "parser definitions stay in each skill." If a third skill is added, extract this JSON parsing pipeline into `_shared/json_io.py` parameterized by `_JSON_FIELD_TYPES`.
...
- [ ] **Step 5: Skip commit** — `__main__.py` imports `sl_core.main` which doesn't exist yet. Commit together with Task 2 to avoid a broken intermediate state.
...
```bash
git add skills/simulink_edit/__init__.py skills/simulink_edit/scripts/__init__.py skills/simulink_edit/__main__.py skills/simulink_edit/scripts/sl_core.py
git commit -m "feat(edit): scaffold simulink_edit with sl_core.py schema and parser"
```
